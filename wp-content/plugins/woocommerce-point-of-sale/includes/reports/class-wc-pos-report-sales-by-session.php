...
/**
 * WC_POS_Report_Sales_By_Session.
 */
class WC_POS_Report_Sales_By_Session extends WC_Admin_Report {
		...
		$this->report_data->orders = (array) $this->get_order_report_data(
			array(
				...
				'where_meta'          => array(
					...
					array(
						'meta_key'   => array( 'wc_pos_id_session' ),
	...
	public function get_chart_legend() {
		...
		$legend[] = array(
			...
			'title'            => sprintf( __( '%s gross sales from this session', 'woocommerce-point-of-sale' ), '<strong>' . wc_price( $data->total_sales ) . '</strong>' ),
		...
		$legend[] = array(
			...
			'title'            => sprintf( __( '%s net sales from this session', 'woocommerce-point-of-sale' ), '<strong>' . wc_price( $data->net_sales ) . '</strong>' ),
		...
		$legend[] = array(
			...
			'title'            => sprintf( __( '%s sales from this session for saved orders', 'woocommerce-point-of-sale' ), '<strong>' . wc_price( $data->saved_sales ) . '</strong>' ),
	...
	public function current_filters() {
		...
		foreach ( $this->session_ids as $session_id ) {
			...
			$session = wc_pos_get_session( absint( $session_id ) );
		...
		echo '<p><a class="button" href="' . esc_url( remove_query_arg( 'session_ids' ) ) . '">' . esc_html__( 'Reset', 'woocommerce-point-of-sale' ) . '</a></p>';
	}
	...
	public function products_widget() {
		?>
		<h4 class="section_title"><span><?php esc_html_e( 'Session Search', 'woocommerce-point-of-sale' ); ?></span></h4>
		<div class="section">
			<form method="GET">
				<div>
					<select class="wc-product-search" style="width:203px;" name="session_ids[]" data-placeholder="<?php esc_attr_e( 'Search for a session&hellip;', 'woocommerce-point-of-sale' ); ?>" data-action="wc_pos_json_search_sessions"></select>
		...
		<h4 class="section_title"><span><?php esc_html_e( 'Top Sessions', 'woocommerce-point-of-sale' ); ?></span></h4>
		<div class="section">
			<table cellspacing="0">
				<?php
				$top_sellers = $this->get_order_report_data(
					array(
						'data'         => array(
							...
							'wc_pos_id_session' => array(
								'type'     => 'meta',
								'function' => 'COUNT',
								'name'     => 'count, meta_wc_pos_id_session.meta_value as session_id',
							),
						),
						'where_meta'   => array(
							array(
				...
				if ( $top_sellers ) {
					foreach ( $top_sellers as $session ) {
						$reg      = wc_pos_get_session( absint( $session->session_id ) );
					...
					echo '<tr><td colspan="3">' . esc_html__( 'No sessions found in range', 'woocommerce-point-of-sale' ) . '</td></tr>';
				}
				?>
			</table>
		</div>
	...
	public function get_main_chart() {
		...
		if ( ! $this->session_ids ) {
			...
			<div class="chart-container">
				<p class="chart-prompt"><?php esc_html_e( '&larr; Choose a session to view stats', 'woocommerce-point-of-sale' ); ?></p>
			</div>
			<?php
		} else {
